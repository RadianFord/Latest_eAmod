...
{
	int i, j, amount, new_, w, vt, vp;
	unsigned short nameid;
	struct npc_data *nd = (struct npc_data *)map_id2bl(sd->npc_shopid);

	if( !nd || (nd->subtype != NPCTYPE_SPSHOP && nd->subtype != NPCTYPE_CASHSHOP) )
		return 1;
...
int npc_unload(struct npc_data* nd, bool single) {
	...
	if( (nd->subtype == NPCTYPE_SHOP || nd->subtype == NPCTYPE_CASHSHOP || nd->subtype == NPCTYPE_ITEMSHOP || nd->subtype == NPCTYPE_POINTSHOP || nd->subtype == NPCTYPE_SPSHOP) && nd->src_id == 0) //src check for duplicate shops [Orcao]
	{
...
static const char* npc_parse_shop(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	if( type == NPCTYPE_SPSHOP && cashitem >= 0 && itemdb_exists(cashitem) == NULL )
	{
	...
	if( type == NPCTYPE_CASHSHOP && !strcasecmp(w2,"cashshop") )
	{ // Default Values
...
const char* npc_parse_duplicate(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	// get placement
	if( (type == NPCTYPE_SHOP || type == NPCTYPE_CASHSHOP || type == NPCTYPE_ITEMSHOP || type == NPCTYPE_POINTSHOP || type == NPCTYPE_SCRIPT || type == NPCTYPE_SPSHOP) && strcmp(w1, "-") == 0 ) {// floating shop/chashshop/itemshop/pointshop/script
		x = y = dir = 0;
		m = -1;
	} else {
		char mapname[32];
